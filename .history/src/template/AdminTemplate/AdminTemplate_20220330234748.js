...
export const AdminTemplate = (props) => {
  ...
  const renderPath = () => {
    ...
    for (let i = 0; i < path.length; i++) {
      let data = (
        <Breadcrumb.Item>
          <NavLink to={`/${path[i - 1]}/${path[i]}`}>{path[i]}</NavLink>
        </Breadcrumb.Item>
      );
      arrR.push(data);
    }
  ...
  return (
    <Route
      ...
      exact
      render={(propsRoute) => {
        return localStorage.getItem(TYPE_USER) === '"QuanTri"' ? (
          <Fragment>
            <Layout style={{ minHeight: "100vh" }}>
              <Sider collapsible collapsed={collapsed} onCollapse={onCollapse}>
                ...
                <Menu
                  theme="dark"
                  defaultSelectedKeys={[defaultKey(path[2])]}
                  mode="inline"
                  style={{ height: "200px" }}
                >
                  ...
                  >
                    <Menu.Item key="7">
                      <NavLink to="/admin/list-film">Danh Sách Phim</NavLink>
                    </Menu.Item>
                  </SubMenu>
                  <SubMenu
                    key="sub3"
                    icon={<HddOutlined />}
                    title="Quản Lý Phòng Vé"
                  >
                    <Menu.Item key="12">
                      <NavLink to="/admin/movie-shedule">
                        Lịch Chiếu Phim
                      </NavLink>{" "}
                    </Menu.Item>
                  </SubMenu>
                  ...
                  >
                    <Menu.Item key="14">
                      <NavLink to="/admin/list-cinema">
                        Thông Tin Cụm Rạp
                      </NavLink>
                    </Menu.Item>
                    <Menu.Item key="15">
                      <NavLink to="/admin/shedule-cinema">
